...
if [ "$latest_tag_available" = "true" ]; then
  ...
  docker buildx build \
    ...
    --output type=image,\"name=$tagged_registry_image,$latest_registry_image\",oci-mediatypes=true,compression=zstd,compression-level=22,force-compression=true,push=true \
    $INPUT_ARGS -f "$file" \
    "$INPUT_PATH";
  echo "::endgroup::"
